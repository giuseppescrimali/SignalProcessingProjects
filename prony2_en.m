% Complex Exponential method including STABILIZATION DIAGRAMS
...
%  plot(t,y,'r')
  ...
  while continua==1
    ...
    % Number of modes
    str4=[' MAXIMUM number of modes [<=' int2str(l/4) ']: '];
    for n=2:input(str4)
   % n=input(str4);
 ...
 %     end
   ...
   % plot(n,caratt(:,2),'*'),hold on
    plot(caratt(:,2),n,'*'),hold on
    
    %disp('smorz.[%] - freq.[Hz] - cost.modale - fase[°]')
    %disp(caratt)
 end
% xlabel('number of modes'),ylabel('freq. [Hz]'),title('frequency stabilization diagram')
xlabel('freq. [Hz]'),ylabel('number of modes'),title('frequency stabilization diagram')
 Continue
    continua=menu('Continue?','Yes','NO');
    if continua==1, clf,clear caratt,end
